...
def agregar_grupos(_unused):
    ...
    while respuesta_grupo == "SI":
        ...
        respuesta = "SI"
        while respuesta == "SI":
            nombre_alumno = input(f"\U0001F464  Nombre del alumno: ").strip()
            if nombre_alumno:
            ...
            respuesta = obtener_SI_NO("¿Agregar otro alumno? (SI/NO): ")
...
def eliminar_grupos(_unused):
    ...
    if opcion == "1":
        ...
        if not g:
            ...
            return
        confirmar = obtener_SI_NO("¿Eliminar este grupo y sus alumnos? (SI/NO): ")
        if confirmar == "SI":
        ...
        if not candidatos:
            ...
            return
        a = candidatos[0]
        confirmar = obtener_SI_NO(f"¿Eliminar a «{a['nombre']}»? (SI/NO): ")
        if confirmar == "SI":
...
def modificar(_unused):
    ...
    if op == "1":
        ...
        if not nuevo:
            ...
            return
        if mgrupo.renombrar(g["id"], nuevo):
            print("\U00002705  Grupo renombrado correctamente.")
        else:
            print("\U0000274C  No se pudo renombrar (posible duplicado).")
        ...
        if malumno.renombrar(a["id"], nuevo_nombre):
            ...
            print("\U0000274C  No se pudo renombrar al alumno.")
        ...
        if not g_destino:
            crear = obtener_SI_NO("\U0001F195  El grupo destino no existe. ¿Crearlo? (SI/NO): ")
            if crear == "SI":
        ...
        if malumno.mover_de_grupo(a["id"], g_destino_id):
            ...
            print("\U0000274C  No se pudo mover al alumno.")
    else:
        print("\U000026A0\U0000FE0F  Opción inválida.")
...
def vaciar_grupos(_unused):
    ...
    resp = input("¿Deseas continuar? (SI/NO): ").strip().upper()
    if resp == "SI":
